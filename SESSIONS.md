...
|---|---|---|---|
| 2026-08-01 | Riu | 🔥 +100 Together Dares | `daretogetherx` 25 → **125**, so 💞 Together + 🔥 Dare now draws from **150** — `tfdPool()` flattens both together arrays into one pool, so the `x` deck is the spicier half of one deck, not a separate mode. BANK 345 → 445. Appended at the END of the array, per the convention that keeps `BANK` and the DB in the same order. **The daily question is untouched**: `dailyQuestion()` pools from `QOTD_CATS` (funny + romantic + ldr) only, so nothing reshuffles — the add-daily-questions skill's blanket 'this reshuffles every future day' warning does not apply to a Dare deck. **Found and fixed on the way: `supabase/append_questions.py` has been broken since the RLS lockdown.** It sends the anon key, but `questions` is `to authenticated using (public.is_us())` with no anon policy — so the SELECT returns an empty array *without erroring*, the script concluded the table was empty, and the dry run offered to append the entire bank. Against a populated DB that would have duplicated every question and desynced the online and offline decks permanently. It now reads a user JWT from `LR_TOKEN` and **refuses an empty anon read** rather than guessing |
| 2026-08-01 | Riu | 👤 Assign who's who in 👥 Who can sign in | The 'I'm' picker in ⚙️ Settings is **gone**. Every allowed address in 👥 Who can sign in now carries a Lucia/Riu dropdown, and that's the whole identity UI — which means **either of you can assign BOTH addresses from your own phone**, something a self-service picker could never do, and it's the screen that already decides who these people are. Storage is unchanged (`settings.account_<who>`, riding the one boot fetch, no migration). `meAssign()` is the only writer; `meAssignSelect()` builds the control in js/me.js and js/auth.js drops it into the row — auth.js owns the list, me.js owns what the answer means. **One address is never both of you**: assigning an address that's currently the other person deletes that row, or `meFromAccount()` answers with whichever it scanned first. `ME_LOCKS` still bites, but only on YOUR OWN row — reassigning the other address can't move the phone in your hand. `meRender()` kept its name (meBroadcast, meAdopt and renderWhoAmIRows all reach it) and now repaints the dropdowns, which is real work: a duel starting mid-session is what greys them out. **Deliberate loss:** signed out there is now no way to say who you are except `#me=` in the URL — everything this identity feeds needs the database anyway, and that needs a sign-in |
| 2026-08-01 | Riu | 👤 The account remembers who you are | Follow-up to "One 'I'm'": the email→person mapping was hardcoded in `AUTH_ALLOWED` (js/auth.js), read **by array position**, and still carried `lucia@example.com` — so changing who's who meant a code edit and a deploy, and Lucia's phone got no benefit at all. Now **picking your name while signed in IS the mapping**: it writes `settings.account_<who> = <email>` and never asks again, on any phone. No separate form, no extra tap. Stored in `settings` rather than a new table so it rides the ONE boot fetch (`loadSettings` → `meAdopt`) — no extra request, **no migration**. `AUTH_ALLOWED` stays as the fallback for a phone that hasn't seen those rows yet, and the learned mapping beats it. Re-picking on a linked account **deletes the other slot** — one address must never be both of you, or `meFromAccount()` answers with whichever it scanned first. `🔓 Not me` unlinks; **tapping the name you're already on re-links**, because otherwise the hint telling you to do that was a lie (found in testing). Also swept two stale pointers the previous PR missed: 📍 Location still said "the chips in 🎮 Games" and 20 Questions' first-frame markup still said "Pick who you are" |
| 2026-08-01 | Riu | 👤 **One "I'm"**, in ⚙️ Settings | The app asked who you were in FOUR places — Word Duel, 20 Questions, ✍️ Answer & compare and 📅 the calendar each carried their own "I'm playing as" row, all writing the same `#me`, while 📍 Location just refused to work until you'd found one of them. Now: one picker in ⚙️ Settings, and **usually no picker at all** — signed in, `meFromAccount()` reads the Google address through `AUTH_ALLOWED`, whose **order is load-bearing (Riu `[0]`, Lucia `[1]`)**; reorder it and you swap the two of them everywhere. It returns null rather than guessing while that list still holds its `@example.com` placeholder. **A Settings picker alone wouldn't have fixed this**: `start_url` is `./`, so every cold launch of the home-screen app starts with no hash and the answer was gone again — deriving it from the account is what actually ends the nagging, and it stores nothing (no table, no settings row, re-derived each launch). `wdMe` → `meWho`, moved out of js/duel.js into new **js/me.js**: identity was never the duel's to own, which is why js/where.js used to reach into the duel to ask. `ME_LOCKS` carries the duel's side lock over to the new picker so you still can't swap mid-match, with the rule left in js/duel.js where it's true. `renderWhoAmIRows()` kept its name — several callers still fire it — and now just repaints the Settings picker. **No migration** |
| 2026-07-31 | Riu | **📅 Our calendar** on Home | A month grid on the Home card that opens into a sheet you can both write to. **Colour = who added it**, nothing else — `--who-riu` / `--who-lucia` are FIXED in css/calendar.css, not derived from `--accent`, because a theme repainting the accent would otherwise repaint people's identities. Days are `"YYYY-MM-DD"` strings and times are free text: two timezones mean a timestamp renders as a different DAY on the two phones (the bug 🍜 Food paid for once). `calMinutes()` parses "6am"/"19:30" so a day sorts by clock, not alphabetically. Its own TABLE (`supabase/calendar.sql`) rather than a `settings` JSON row — two people add independently and a read-modify-write blob loses whoever saves second. **Question of the Day + Answer & compare moved to 🎭 Talk** rather than being deleted with the Home slot they occupied |
| 2026-07-26 | Riu | 🔔 **Notifications** — the other phone buzzes | Web Push, which needed the one thing the board said we'd never add. **`sw.js` exists now, and has NO `fetch` handler** — the old "deliberately NO service worker" rule was about *caching* breaking golden rule 4, and a worker that intercepts nothing can't serve a stale shell. That distinction is now written into CLAUDE.md and the skill, because the next session will otherwise read the old rule and revert this. **iOS: Home Screen only** (16.4+), permission only from a tap, and no second prompt if denied — so it's a deliberate toggle in ⚙️ Settings, never an ask on boot. **The push carries no payload**: encryption (RFC 8291) would mean a dependency, so every notification reads "Something new 💞" and the app shows what changed — and *nothing about us goes through Apple's servers*. `push_subs` keeps `p256dh`/`auth` anyway so adding lock-screen text later is a sender change, not a re-subscribe. The actor comes from the **verified JWT**, never the body, so neither of us can make the other's phone buzz on demand; `api/notify.js` reads the table with the caller's own token (no service-role key anywhere) and prunes endpoints that 404/410. `api/_gate.js` is the auth check extracted from `api/claude.js` — security code shouldn't exist twice. Fires from three places (`js/food.js` upload, `sdAdd`, `tpSave`), fire-and-forget so it can never fail the thing it announces. Needs `supabase/push.sql` + three VAPID env vars; `VAPID_PUBLIC` empty ⇒ toggle hidden, so it ships safely before the keys exist |
...
timeline), `wd*` (Word Duel), `cp*` (couple photo), `cy*` (🌙 Moon calendar), `cal*` (📅 Our calendar, table `calendar_events`), `auth*` (Google sign-in).
